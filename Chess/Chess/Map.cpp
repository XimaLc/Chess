#include "Map.h"

void Map::setTexture(std::string _file, bool _isPerso)
{
    texture.loadFromFile(_file);
    isPerso = _isPerso;
    setSprite();
}

void Map::setSprite()
{
    sprite.setTexture(texture);
}

void Map::fill()
{
    map.clear();
    int i = 0;
    for (int y = 0; y < 8; y++)
    {
        if (y % 2 == 0)
            i = 1;
        if (y % 2 > 0)
            i = 0;
        for (int x = 0; x < 8; x++)
        {
            map.push_back({ 0, i });
            i++;
            if (i == 2)
                i = 0;
        }
    }
}

void Map::white()
{
    map.clear();
    for (int i = 0; i < 48; i++)
    {
        map.push_back(sf::Vector2i{ none, 0 });
    }
    if (isPerso)
    {
        map.push_back(sf::Vector2i{ pawn, 1 });
        map.push_back(sf::Vector2i{ pawn, 2 });
        map.push_back(sf::Vector2i{ pawn, 3 });
        map.push_back(sf::Vector2i{ pawn, 4 });
        map.push_back(sf::Vector2i{ pawn, 5 });
        map.push_back(sf::Vector2i{ pawn, 6 });
        map.push_back(sf::Vector2i{ pawn, 7 });
        map.push_back(sf::Vector2i{ pawn, 8 });
    }
    else
    {
        for (int i = 48; i < 56; i++)
        {
            map.push_back(sf::Vector2i{ pawn, 0 });
        }
    }
    map.push_back(sf::Vector2i{ rook, 0 });
    map.push_back(sf::Vector2i{knight, 0});
    map.push_back(sf::Vector2i{bishop, 0});
    map.push_back(sf::Vector2i{ queen, 0 });
    map.push_back(sf::Vector2i{ king, 0 });
    map.push_back(sf::Vector2i{ bishop, 0 });
    map.push_back(sf::Vector2i{ knight, 0 });
    map.push_back(sf::Vector2i{ rook, 0 });
}

void Map::black()
{
    map.clear();
    map.push_back(sf::Vector2i{ rook, 0 });
    map.push_back(sf::Vector2i{ knight, 0 });
    map.push_back(sf::Vector2i{ bishop, 0 });
    map.push_back(sf::Vector2i{ king, 0 });
    map.push_back(sf::Vector2i{ queen, 0 });
    map.push_back(sf::Vector2i{ bishop, 0 });
    map.push_back(sf::Vector2i{ knight, 0 });
    map.push_back(sf::Vector2i{ rook, 0 });
    if (isPerso)
    {
        map.push_back(sf::Vector2i{ pawn, 1 });
        map.push_back(sf::Vector2i{ pawn, 2 });
        map.push_back(sf::Vector2i{ pawn, 3 });
        map.push_back(sf::Vector2i{ pawn, 4 });
        map.push_back(sf::Vector2i{ pawn, 5 });
        map.push_back(sf::Vector2i{ pawn, 6 });
        map.push_back(sf::Vector2i{ pawn, 7 });
        map.push_back(sf::Vector2i{ pawn, 8 });
    }
    else
    {
        for (int i = 8; i < 16; i++)
        {
            map.push_back(sf::Vector2i{ pawn, 0 });
        }
    }
    for (int i = 16; i < 64; i++)
    {
        map.push_back(sf::Vector2i{ none, 0 });
    }
}

void Map::draw(sf::RenderWindow& _window, bool isPLayer)
{
    int x = 0;
    int y = 0;
    sf::IntRect rect;

    for (int i = 0; i < 8 * 8; i++)
    {
        if (x == 8)
        {
            x = 0;
            y++;
        }

        if (map[i].y != 0 && isPLayer)
            rect = sf::IntRect(64 * (map[i].y - 1), 64, 64, 64);
        else
            rect = sf::IntRect(64 * map[i].x, 64 * map[i].y, 64, 64);

        sprite.setTextureRect(rect);
        sprite.setPosition(x * 64, y * 64);
        _window.draw(sprite);

        x++;
    }
}

bool Map::showKingMoves(int _x, int _y, Map _opponentMap, sf::Color team)
{
    if (_y < 7)
    {
        if(!isNextCellCheckable(_x, _y + 1, _opponentMap, team))
            if (_opponentMap.returnPiece(_x, _y+1).x != 6)
                editPiece(_x, _y + 1, 8);
            else if (map[(_x) + (_y + 1) * 8].x == 6)
                editPiece(_x, _y + 1, 7);
    }
    if (_y > 0)
    {
        if (!isNextCellCheckable(_x, _y - 1, _opponentMap, team))
            if (_opponentMap.returnPiece(_x, _y - 1).x != 6)
                editPiece(_x, _y - 1, 8);
            else if (map[(_x)+(_y - 1) * 8].x == 6)
                editPiece(_x, _y - 1, 7);
    }
    if (_x < 7)
    {
        if (!isNextCellCheckable(_x + 1, _y, _opponentMap, team))
            if (_opponentMap.returnPiece(_x + 1, _y).x != 6)
                editPiece(_x + 1, _y, 8);
            else if (map[(_x + 1)+(_y) * 8].x == 6)
                editPiece(_x + 1, _y, 7);
    }
    if (_x > 0)
    {
        if (!isNextCellCheckable(_x - 1, _y, _opponentMap, team))
            if (_opponentMap.returnPiece(_x - 1, _y).x != 6)
                editPiece(_x - 1, _y, 8);
            else if (map[(_x - 1) + (_y) * 8].x == 6)
                editPiece(_x - 1, _y, 7);
    }
    if (_y > 0 && _x < 7)
    {
        if (!isNextCellCheckable(_x + 1, _y - 1, _opponentMap, team))
            if (_opponentMap.returnPiece(_x + 1 , _y - 1).x != 6)
                editPiece(_x + 1, _y - 1, 8);
            else if (map[(_x + 1)+(_y - 1) * 8].x == 6)
                editPiece(_x + 1, _y - 1, 7);
    }
    if (_y > 0 && _x > 0)
    {
        if (!isNextCellCheckable(_x - 1, _y - 1, _opponentMap, team))
            if (_opponentMap.returnPiece(_x - 1, _y - 1).x != 6)
                editPiece(_x - 1, _y - 1, 8);
            else if (map[(_x - 1)+(_y - 1) * 8].x == 6)
                editPiece(_x - 1, _y - 1, 7);
    }
    if (_y < 7 && _x < 7)
    {
        if (!isNextCellCheckable(_x + 1, _y + 1, _opponentMap, team))
            if (_opponentMap.returnPiece(_x + 1, _y + 1).x != 6)
                editPiece(_x + 1, _y + 1, 8);
            else if (map[(_x + 1)+(_y + 1) * 8].x == 6)
                editPiece(_x + 1, _y + 1, 7);
    }
    if (_y < 7 && _x > 0)
    {
        if (!isNextCellCheckable(_x - 1, _y + 1, _opponentMap, team))
            if (_opponentMap.returnPiece(_x - 1, _y + 1).x != 6)
                editPiece(_x - 1, _y + 1, 8);
            else if (map[(_x - 1)+(_y + 1) * 8].x == 6)
                editPiece(_x - 1, _y + 1, 7);
    }
    for (int i = 0; i < 64; i++)
    {
        if (map[i].x == 7 || map[i].x == 8)
            return true;
    }
    return false;


}
void Map::showRookMoves(int _x, int _y, Map _opponentMap, bool isChecked)
{
    for (int i = 1; i < 8; i++)
    {
        if (_y-i >= 0)
        {
            int piece = returnPiece(_x, _y - i).x;
            int opponentPiece = _opponentMap.returnPiece(_x, _y - i).x;
            if (piece != 6)
                i = 8;
                
            if (piece == 6 && opponentPiece == 6)
                editPiece(_x, _y - i, 7);


            if (opponentPiece != 6)
            {
                editPiece(_x, _y - i, 8);
                i = 8;
            }

        }
    }
    for (int i = 1; i < 8; i++)
    {
        if (_y+i <= 7)
        {
            int piece = returnPiece(_x, _y + i).x;
            int opponentPiece = _opponentMap.returnPiece(_x, _y + i).x;

            if (piece != 6)
                i = 8;

            if (piece == 6 && opponentPiece == 6)
                editPiece(_x, _y + i, 7);
            if (opponentPiece != 6)
            {
                editPiece(_x, _y + i, 8);
                i = 8;
            }
        }
    }
    for (int i = 1; i < 8; i++)
    {
        if (_x - i >= 0)
        {
            int piece = returnPiece(_x - i, _y).x;
            int opponentPiece = _opponentMap.returnPiece(_x - i, _y).x;
            if (piece != 6)
                i = 8;

            if (piece == 6 && opponentPiece == 6)
                editPiece(_x - i, _y, 7);


            if (opponentPiece != 6)
            {
                editPiece(_x - i, _y, 8);
                i = 8;
            }

        }
    }
    for (int i = 1; i < 8; i++)
    {
        if (_x + i <= 7)
        {
            int piece = returnPiece(_x + i, _y).x;
            int opponentPiece = _opponentMap.returnPiece(_x + i, _y).x;

            if (piece != 6)
                i = 8;

            if (piece == 6 && opponentPiece == 6)
                editPiece(_x + i, _y, 7);
            if (opponentPiece != 6)
            {
                editPiece(_x + i, _y, 8);
                i = 8;
            }
        }
    }
}
void Map::showBishopMoves(int _x, int _y, Map _opponentMap, bool isChecked)
{
    for (int i = 1; i < 8; i++)
    {
        if (_y - i >= 0 && _x - i >= 0)
        {
            int piece = returnPiece(_x - i, _y - i).x;
            int opponentPiece = _opponentMap.returnPiece(_x - i, _y - i).x;
            if (piece != 6)
                i = 8;

            if (piece == 6 && opponentPiece == 6)
                editPiece(_x - i, _y - i, 7);


            if (opponentPiece != 6)
            {
                editPiece(_x - i, _y - i, 8);
                i = 8;
            }

        }
    }
    for (int i = 1; i < 8; i++)
    {
        if (_y + i <= 7 && _x + i <= 7)
        {
            int piece = returnPiece(_x + i, _y + i).x;
            int opponentPiece = _opponentMap.returnPiece(_x + i, _y + i).x;

            if (piece != 6)
                i = 8;

            if (piece == 6 && opponentPiece == 6)
                editPiece(_x + i, _y + i, 7);
            if (opponentPiece != 6)
            {
                editPiece(_x + i, _y + i, 8);
                i = 8;
            }
        }
    }
    for (int i = 1; i < 8; i++)
    {
        if (_y - i >= 0 && _x + i <= 7) 
        {
            int piece = returnPiece(_x + i, _y - i).x;
            int opponentPiece = _opponentMap.returnPiece(_x + i, _y - i).x;
            if (piece != 6)
                i = 8;

            if (piece == 6 && opponentPiece == 6)
                editPiece(_x + i, _y - i, 7 );


            if (opponentPiece != 6)
            {
                editPiece(_x + i, _y - i, 8);
                i = 8;
            }

        }
    }
    for (int i = 1; i < 8; i++)
    {
        if (_y + i <= 7 && _x - i >= 0)
        {
            int piece = returnPiece(_x - i, _y + i).x;
            int opponentPiece = _opponentMap.returnPiece(_x - i, _y + i).x;

            if (piece != 6)
                i = 8;

            if (piece == 6 && opponentPiece == 6)
                editPiece(_x - i, _y + i, 7);
            if (opponentPiece != 6)
            {
                editPiece(_x - i, _y + i, 8);
                i = 8;
            }
        }
    }
}
void Map::showKnightMoves(int _x, int _y, Map _opponentMap, bool isChecked)
{
    if (_y < 6 && _x > 0)
    {
        if (_opponentMap.returnPiece(_x - 1, _y + 2).x != 6)
            editPiece(_x - 1, _y + 2, 8);
        else if (map[(_x - 1) + (_y + 2) * 8].x == 6)
            editPiece(_x - 1, _y + 2, 7);
    }
    if (_y < 6 && _x < 7)
    {
        if (_opponentMap.returnPiece(_x + 1, _y + 2).x != 6)
            editPiece(_x + 1, _y + 2, 8);
        else if (map[(_x + 1) + (_y + 2) * 8].x == 6)
            editPiece(_x + 1, _y + 2, 7);
    }

    if (_y > 1 && _x < 7)
    {
        if (_opponentMap.returnPiece(_x + 1, _y - 2).x != 6)
            editPiece(_x + 1, _y - 2, 8);
        else if (map[(_x + 1) + ((_y - 2) * 8)].x == 6)
            editPiece(_x + 1, _y - 2, 7);
    }

    if (_y > 1 && _x > 0)
    {
        if (_opponentMap.returnPiece(_x - 1, _y - 2).x != 6)
            editPiece(_x - 1, _y - 2, 8);
        else if (map[(_x - 1) + (_y - 2) * 8].x == 6)
            editPiece(_x - 1, _y - 2, 7);
    }

    if (_y > 0 && _x > 1)
    {
        if (_opponentMap.returnPiece(_x - 2, _y - 1).x != 6)
            editPiece(_x - 2, _y - 1, 8);
        else if (map[(_x - 2) + (_y - 1) * 8].x == 6)
            editPiece(_x - 2, _y - 1, 7);
    }

    if (_y > 0 && _x < 6)
    {
        if (_opponentMap.returnPiece(_x + 2, _y - 1).x != 6)
            editPiece(_x + 2, _y - 1, 8);
        else if (map[(_x + 2) + (_y - 1) * 8].x == 6)
            editPiece(_x + 2, _y - 1, 7);
    }

    if (_y < 7 && _x > 1)
    {
        if (_opponentMap.returnPiece(_x - 2, _y + 1).x != 6)
            editPiece(_x - 2, _y + 1, 8);
        else if (map[(_x - 2) + (_y + 1) * 8].x == 6)
            editPiece(_x - 2, _y + 1, 7);
    }

    if (_y < 7 && _x < 6)
    {
        if (_opponentMap.returnPiece(_x + 2, _y + 1).x != 6)
            editPiece(_x + 2, _y + 1, 8);
        else if (map[(_x + 2) + (_y + 1) * 8].x == 6)
            editPiece(_x + 2, _y + 1, 7);
    }
}
void Map::showPawnMoves(int _x, int _y, sf::Color _team, Map _opponentMap, bool isChecked)
{
    if (_team == sf::Color::White)
    {
        if (_x < 7 && _y > 0)
            if (_opponentMap.returnPiece(_x + 1, _y - 1).x != 6)
                editPiece(_x + 1, _y - 1, 8);

        if (_x > 0 && _y > 0)
            if (_opponentMap.returnPiece(_x - 1, _y - 1).x != 6)
                editPiece(_x - 1, _y - 1, 8);

        if (_y == 6)
            if (_opponentMap.returnPiece(_x, _y - 1).x == 6 && returnPiece(_x, _y - 1).x == 6)
            {
                editPiece(_x, _y - 1, 7);
                if (_opponentMap.returnPiece(_x, _y - 2).x == 6 && returnPiece(_x, _y - 2).x == 6)
                    editPiece(_x, _y - 2, 7);
            }
        
        if (_y < 6 && _y > 0)
            if (_opponentMap.returnPiece(_x, _y - 1).x == 6 && returnPiece(_x, _y - 1).x == 6)
                editPiece(_x, _y - 1, 7);
    }

    if (_team == sf::Color::Black)
    {
        if (_x > 0 && _y < 7)
            if (_opponentMap.returnPiece(_x - 1, _y + 1).x != 6)
                editPiece(_x - 1, _y + 1, 8);

        if (_x < 7 && _y < 7)
            if (_opponentMap.returnPiece(_x + 1, _y + 1).x != 6)
                editPiece(_x + 1, _y + 1, 8);

        if (_y == 1)
            if (_opponentMap.returnPiece(_x, _y + 1).x == 6 && returnPiece(_x, _y + 1).x == 6)
            {
                editPiece(_x, _y + 1, 7);
                if (_opponentMap.returnPiece(_x, _y + 2).x == 6 && returnPiece(_x, _y + 2).x == 6)
                    editPiece(_x, _y + 2, 7);
            }
        
        if (_y < 7)
            if (_opponentMap.returnPiece(_x, _y + 1).x == 6 && map[_x + (_y + 1) * 8].x == 6)
                editPiece(_x, _y + 1, 7);
    }

}
void Map::resetPossibleMoves()
{
    for (int y = 0; y < 8; y++)
    {
        for (int x = 0; x < 8; x++)
        {
            if (map[x + y * 8].x == 7 || map[x + y * 8].x == 8)
            {
                map[x + y * 8].x = 6;
            }
        }
    }
}
void Map::editPiece(int _x, int _y, sf::Vector2i _newPiece)
{
    map[_x + _y * 8] = _newPiece;
}
void Map::editPiece(sf::Vector2i _pos, sf::Vector2i _newPiece)
{
    map[_pos.x + _pos.y * 8] = _newPiece;
}

void Map::editPiece(int _x, int _y, int _newPiece)
{
    map[_x + _y * 8].x = _newPiece;
    map[_x + _y * 8].y = 0;
}
void Map::editPiece(sf::Vector2i _pos, int _newPiece)
{
    map[_pos.x + _pos.y * 8].x = _newPiece;
    map[_pos.x + _pos.y * 8].y = 0;
}
sf::Vector2i Map::find(int _pieceId)
{
    for (int y = 0; y < 8; y++)
    {
        for (int x = 0; x < 8; x++)
        {
            if (returnPiece(x, y).x == _pieceId)
                return { x, y };
        }
    }
    return { -1, -1 };
}

bool Map::isCheckedByRookAndMoitieDeQueen(int _x, int _y, Map opponentMap)
{
    sf::Vector2i opponentNearest = getNearest(_x, _y, opponentMap, 0);
    sf::Vector2i myNearest = getNearest(_x, _y, *this, 0);
    if(isCheckOrProtected(_x, _y, myNearest, opponentNearest, opponentMap, {1, 2}))
        return true;

    opponentNearest = getNearest(_x, _y, opponentMap, 2);
    myNearest = getNearest(_x, _y, *this, 2);
    if (isCheckOrProtected(_x, _y, myNearest, opponentNearest, opponentMap, { 1, 2 }))
        return true;
        
    opponentNearest = getNearest(_x, _y, opponentMap, 4);
    myNearest = getNearest(_x, _y, *this, 4);
    if (isCheckOrProtected(_x, _y, myNearest, opponentNearest, opponentMap, { 1, 2 }))
        return true;
        
    opponentNearest = getNearest(_x, _y, opponentMap, 6);
    myNearest = getNearest(_x, _y, *this, 6);
    if (isCheckOrProtected(_x, _y, myNearest, opponentNearest, opponentMap, { 1, 2 }))
        return true;


    return false;
}
bool Map::isCheckedByBishopAndMoitieDeQueen(int _x, int _y, Map opponentMap)
{
    sf::Vector2i opponentNearest = getNearest(_x, _y, opponentMap, 1);
    sf::Vector2i myNearest = getNearest(_x, _y, *this, 1);
    if (isCheckOrProtected(_x, _y, myNearest, opponentNearest, opponentMap, { 1, 3 }))
        return true;

    opponentNearest = getNearest(_x, _y, opponentMap, 3);
    myNearest = getNearest(_x, _y, *this, 3);
    if (isCheckOrProtected(_x, _y, myNearest, opponentNearest, opponentMap, { 1, 3 }))
        return true;

    opponentNearest = getNearest(_x, _y, opponentMap, 5);
    myNearest = getNearest(_x, _y, *this, 5);
    if (isCheckOrProtected(_x, _y, myNearest, opponentNearest, opponentMap, { 1, 3 }))
        return true;

    opponentNearest = getNearest(_x, _y, opponentMap, 7);
    myNearest = getNearest(_x, _y, *this, 7);
    if (isCheckOrProtected(_x, _y, myNearest, opponentNearest, opponentMap, { 1, 3 }))
        return true;

    return false;
}
bool Map::isCheckedByPawn(int _x, int _y, Map opponentMap, sf::Color team)
{
    if (team == sf::Color::White)
    {
        if (_y < 7)
        {
            if(_x > 0)
                if (opponentMap.returnPiece(_x - 1, _y + 1).x == pawn)
                    return true;
            if(_x < 7)
                if (opponentMap.returnPiece(_x + 1, _y + 1).x == pawn)
                    return true;
        }
        return false;   
    }
    else if (team == sf::Color::Black)
    {
        if (_y < 7)
        {
            if(_x > 0)
                if (opponentMap.returnPiece(_x - 1, _y + 1).x == pawn)
                    return true;
            if(_x < 7)
                if (opponentMap.returnPiece(_x + 1, _y + 1).x == pawn)
                    return true;
        }
        return false;
    }
    return false;
}
bool Map::isCheckedByKnight(int _x, int _y, Map opponentMap)
{
    if (_x > 1)
    {
        if (_y > 0)
            if (opponentMap.returnPiece(_x - 2, _y - 1).x == knight)
                return true;
        if (_y < 7)
            if (opponentMap.returnPiece(_x - 2, _y + 1).x == knight)
                return true;
    }
    if (_x < 6)
    {
        if (_y > 0)
            if (opponentMap.returnPiece(_x + 2, _y - 1).x == knight)
                return true;
        if (_y < 7)
            if (opponentMap.returnPiece(_x + 2, _y + 1).x == knight)
                return true;
    }
    if (_y > 1)
    {
        if (_x > 0)
            if (opponentMap.returnPiece(_x - 1, _y - 2).x == knight)
                return true;
        if (_x < 7)
            if (opponentMap.returnPiece(_x + 1, _y - 2).x == knight)
                return true;
    }
    if (_y < 6)
    {
        if (_x > 0)
            if (opponentMap.returnPiece(_x - 1, _y + 2).x == knight)
                return true;
        if (_x < 7)
            if (opponentMap.returnPiece(_x + 1, _y + 2).x == knight)
                return true;
    }
    return false;
}
bool Map::isCheckedByKing(int _x, int _y, Map opponentMap)
{
    if (_y < 7)
    {
        if (opponentMap.returnPiece(_x, _y + 1).x == king)
            return true;

        if (_x > 0)
            if (opponentMap.returnPiece(_x - 1, _y + 1).x == king)
                return true;
        if (_x < 7)
            if (opponentMap.returnPiece(_x + 1, _y + 1).x == king)
                return true;
    }
    if (_y > 0)
    {
        if (opponentMap.returnPiece(_x, _y - 1).x == king)
            return true;

        if (_x < 7)
            if (opponentMap.returnPiece(_x + 1, _y + 1).x == king)
                return true;
        if (_x > 0)
            if (opponentMap.returnPiece(_x - 1, _y + 1).x == king)
                return true;
    }
    if(_x > 0)
        if (opponentMap.returnPiece(_x - 1, _y).x == king)
            return true;
    
    if(_x < 7)
        if (opponentMap.returnPiece(_x + 1, _y).x == king)
            return true;

    return false;
}

sf::Vector2i Map::getNearest(int _x, int _y, Map map, int _type)
{
    if (_type == 0) // Au dessus
    {
        for (int i = 1; i < 8; i++)
        {
            if (_y - i >= 0)
            {
                if (map.returnPiece(_x, _y - i).x != 6 && map.returnPiece(_x, _y - i).x != 7 && map.returnPiece(_x, _y - i).x != 8)
                    return { _x, _y - i };
            }
            else
                return { -1, -1 };
        }
        return { -1, -1 };
    }
    if (_type == 1) // Au dessus / Droite
    {
        for (int i = 1; i < 8; i++)
        {
            if (_y - i >= 0 && _x + i <= 7)
            {
                if (map.returnPiece(_x + i, _y - i).x != 6 && map.returnPiece(_x + i, _y - i).x != 7 && map.returnPiece(_x + i, _y - i).x != 8)
                    return { _x + i, _y - i };
            }
            else
                return { -1, -1 };
        }
        return { -1, -1 };
    }
    if (_type == 2) // A droite
    {
        for (int i = 1; i < 8; i++)
        {
            if (_x + i <= 7)
            {
                if (map.returnPiece(_x + i, _y).x != 6 && map.returnPiece(_x + i, _y).x != 7 && map.returnPiece(_x + i, _y).x != 8)
                    return { _x + i, _y };
            }
            else
                return { -1, -1 };
        }
        return { -1, -1 };
    }
    if (_type == 3) // En bas / Droite
    {
        for (int i = 1; i < 8; i++)
        {
            if (_y + i <= 7 && _x + i <= 7)
            {
                if (map.returnPiece(_x + i, _y + i).x != 6 && map.returnPiece(_x + i, _y + i).x != 7 && map.returnPiece(_x + i, _y + i).x != 8)
                    return { _x + i, _y + i };
            }
            else
                return { -1, -1 };
        }
        return { -1, -1 };
    }
    if (_type == 4) // En bas
    {
        for (int i = 1; i < 8; i++)
        {
            if (_y + i <= 7)
            {
                if (map.returnPiece(_x, _y + i).x != 6 && map.returnPiece(_x, _y + i).x != 7 && map.returnPiece(_x, _y + i).x != 8)
                    return { _x, _y + i };
            }
            else
                return { -1, -1 };
        }
        return { -1, -1 };
    }
    if (_type == 5) // En bas / Gauche
    {
        for (int i = 1; i < 8; i++)
        {
            if (_y + i <= 7 && _x - i >= 0)
            {
                if (map.returnPiece(_x - i, _y + i).x != 6 && map.returnPiece(_x - i, _y + i).x != 7 && map.returnPiece(_x - i, _y + i).x != 8)
                    return { _x - i, _y + i };
            }
            else
                return { -1, -1 };
        }
        return { -1, -1 };
    }
    if(_type == 6) // A gauche
    {
        for (int i = 1; i < 8; i++)
        {
            if (_x - i >= 0)
            {
                if (map.returnPiece(_x - i, _y).x != 6 && map.returnPiece(_x - i, _y).x != 7 && map.returnPiece(_x - i, _y).x != 8)
                    return { _x - i, _y };
            }
            else
                return { -1, -1 };
        }
        return { -1, -1 };
    }
    if (_type == 7) // En haut / Gauche
    {
        for (int i = 1; i < 8; i++)
        {
            if (_y - i >= 0 && _x - i >= 0)
            {
                if (map.returnPiece(_x - i, _y - i).x != 6 && map.returnPiece(_x - i, _y - i).x != 7 && map.returnPiece(_x - i, _y - i).x != 8)
                    return { _x - i, _y - i };
            }
            else
                return { -1, -1 };
        }
        return { -1, -1 };
    }
}
bool Map::isCheckOrProtected(int _x, int _y, sf::Vector2i _player, sf::Vector2i _opponent, Map _opponentMap, sf::Vector2i _checkers)
{
    if (_opponent != sf::Vector2i{ -1, -1 })
    {
        if (_opponentMap.returnPiece(_opponent).x == _checkers.x || _opponentMap.returnPiece(_opponent).x == _checkers.y)
        {
            if (_player != sf::Vector2i{ -1, -1 })
            {
                    if (GetDistance({ _x, _y }, _opponent) < GetDistance({ _x, _y }, _player))
                        return true;
                return false;
            }
            return true;
        }
    }
    return false;
}

bool Map::isNextCellCheckable(int _x, int _y, Map opponentMap, sf::Color team)
{
    bool checked = isCheckedByRookAndMoitieDeQueen(_x, _y, opponentMap);
    if (!checked)
        checked = isCheckedByBishopAndMoitieDeQueen(_x, _y, opponentMap);
    if (!checked)
        checked = isCheckedByPawn(_x, _y, opponentMap, team);
    if (!checked)
        checked = isCheckedByKnight(_x, _y, opponentMap);

    return checked;
}

sf::Vector2i Map::returnPiece(int _x, int _y)
{
    return map[_x + _y * 8];
}

sf::Vector2i Map::returnPiece(sf::Vector2i _pos)
{
    return map[_pos.x + _pos.y * 8];
}

Map::Map()
{
    initTools();
}